...
### Manuscript (ETC)
- `ETC_DRAFT.md` — lane-assembled draft manuscript (not yet submission-ready).
...
- Remaining: evidence-based “Scopus indexed yes/no” (via Scopus source list or another auditable source) if required by the target journal.
...
- Remaining: if the target journal expects more, add a small number of philosophy-of-biology references specifically on evidence for common ancestry (beyond the minimal anchors).

5) Math/formal lane
- Status: type-check + diagnosis + constraint + restricted revision walkthrough exists.
- Remaining: polish into a “formal rule block” section in the manuscript (tight definitions + notation).

6) Reviewer lane
- Status: stress-test + rejection risks captured.
- Remaining: implement the reviewer “minimum publishability” items as needed for the target journal.

7) Synthesis lane
- Status: assembled into `ETC_DRAFT.md`.
...
- Formal section could be tightened into journal-ready prose.

## Next actions (recommended order)

1) Choose target journal + citation style and apply formatting consistently in `ETC_DRAFT.md`.
2) Tighten the formal model section (definitions + a single diagnosis/revision algorithm box).
